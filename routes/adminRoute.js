...
const DB = "mongodb://localhost:27017/mk_packing";
...
route
  .route("/add_product")
  .get(isLoggedIn.isLoggedIn, adminController.add_product);
...
route.post(
  ...
  async (req, resp) => {
    ...
    const regular_image = JSON.parse(
      JSON.stringify(JSON.parse(JSON.stringify(req.files)).regular_image)
    )[0].filename;
    const mobile_image = JSON.parse(
      JSON.stringify(JSON.parse(JSON.stringify(req.files)).mobile_image)
    )[0].filename;
...
route
  .route("/view_product")
  .get(isLoggedIn.isLoggedIn, adminController.view_product);
...
route
  ...
  .get(isLoggedIn.isLoggedIn, adminController.edit_product);
...
route.post("/update_product/:id", upload, async (req, resp) => {
  ...
  if (req.files) {
    if (JSON.parse(JSON.stringify(req.files)).regular_image !== undefined) {
      const regular_image = JSON.parse(JSON.stringify(req.files))
        .regular_image[0].filename;
      ...
      JSON.parse(JSON.stringify(req.files)).mobile_image !== undefined
    ) {
      const mobile_image = JSON.parse(JSON.stringify(req.files)).mobile_image[0]
        .filename;
...
route
  .route("/manage_logo")
  .get(isLoggedIn.isLoggedIn, adminController.manage_logo);
...
route
  .route("/delete_logo")
  .post(isLoggedIn.isLoggedIn, adminController.delete_logo);

route
  .route("/edit_logo/:id")
  .get(isLoggedIn.isLoggedIn, adminController.edit_logo);
...
route
  .route("/add_our_process")
  .get(isLoggedIn.isLoggedIn, adminController.add_our_process);

route.post(
  "/insert_our_process",
  singleImageUpload.single("process_img"),
  async (req, resp) => {
...
route
  .route("/manage_our_process")
  .get(isLoggedIn.isLoggedIn, adminController.manage_our_process);
route
  .route("/delete_our_process")
  .post(isLoggedIn.isLoggedIn, adminController.delete_our_process);
route
  .route("/edit_our_process/:id")
  .get(isLoggedIn.isLoggedIn, adminController.edit_our_process);

route.post(
  "/update_our_process/:id",
  singleImageUpload.single("process_img"),
  async (req, resp) => {
    const id = req.params.id;
    const { process_name, process_description } = req.body;
...
route
  .route("/add_contact")
  .get(isLoggedIn.isLoggedIn, adminController.add_contact);
route
  .route("/insert_contact")
  .post(isLoggedIn.isLoggedIn, adminController.insert_contact);
route
  .route("/manage_contact")
  .get(isLoggedIn.isLoggedIn, adminController.manage_contact);
route
  .route("/edit_contact/:id")
  .get(isLoggedIn.isLoggedIn, adminController.edit_contact);
route
  .route("/update_contact/:id")
  .post(isLoggedIn.isLoggedIn, adminController.update_contact);
route
  .route("/delete_contact")
  .post(isLoggedIn.isLoggedIn, adminController.delete_contact);

//jumbotron

route
  .route("/add_jumbotron")
  .get(isLoggedIn.isLoggedIn, adminController.add_jumbotron);
route.post(
  "/insert_jumbotron",
  jumbotronUpload.fields([
    {
  ...
  async (req, resp) => {
    ...
    const left_top =
      "images/jumbotron/" +
      JSON.parse(JSON.stringify(req.files)).jumbotron_left_top[0].filename;
    const left_center_blur =
      "images/jumbotron/" +
      JSON.parse(JSON.stringify(req.files)).jumbotron_left_center_blur[0]
        .filename;
    const left_bottom =
      "images/jumbotron/" +
      JSON.parse(JSON.stringify(req.files)).jumbotron_left_bottom[0].filename;
    const right_top =
      "images/jumbotron/" +
      JSON.parse(JSON.stringify(req.files)).jumbotron_right_top[0].filename;
    const right_bottom =
      "images/jumbotron/" +
      JSON.parse(JSON.stringify(req.files)).jumbotron_right_bottom[0].filename;

    const jumbotron_img_1 =
      "images/jumbotron/" +
      JSON.parse(JSON.stringify(req.files)).jumbotron_img_1[0].filename;
    const jumbotron_img_2 =
      "images/jumbotron/" +
      JSON.parse(JSON.stringify(req.files)).jumbotron_img_2[0].filename;
    const jumbotron_img_3 =
      "images/jumbotron/" +
      JSON.parse(JSON.stringify(req.files)).jumbotron_img_3[0].filename;
    const jumbotron_img_4 =
      "images/jumbotron/" +
      JSON.parse(JSON.stringify(req.files)).jumbotron_img_4[0].filename;

    const jumbotron = Jumbotron({
      heading: heading,
      sub_heading: sub_heading,
      image1: jumbotron_img_1,
      image2: jumbotron_img_2,
      image3: jumbotron_img_3,
...
route.post(
  ...
  async (req, resp) => {
    const left_image =
      "images/fact/" +
      JSON.parse(JSON.stringify(req.files)).left_image[0].filename;
    const main_image =
      "images/fact/" +
      JSON.parse(JSON.stringify(req.files)).main_image[0].filename;
    const {
      left_top_heading,
      left_top_sub_heading,
      left_top_svg,
      left_bottom_heading,
      left_bottom_sub_heading,
      left_bottom_svg,
      right_top_heading,
      right_top_sub_heading,
      right_top_svg,
      right_bottom_heading,
      right_bottom_sub_heading,
      right_bottom_svg,
    } = req.body;
...
route
  .route("/manage_fact")
  .get(isLoggedIn.isLoggedIn, adminController.manage_fact);
route
  .route("/edit_fact/:id")
  .get(isLoggedIn.isLoggedIn, adminController.edit_fact);
...
route
  .route("/delete_fact/")
  .post(isLoggedIn.isLoggedIn, adminController.delete_fact);
...
route.post(
  ...
  async (req, resp) => {
    ...
    const right_image =
      "images/about/" +
      JSON.parse(JSON.stringify(req.files)).right_image[0].filename;
...
route
  .route("/manage_about")
  .get(isLoggedIn.isLoggedIn, adminController.manage_about);
route
  .route("/edit_about/:id")
  .get(isLoggedIn.isLoggedIn, adminController.edit_about);
...
route.post(
  "/insert_why_choose",
  whyChooseUpload.single("right_image"),
  async (req, resp) => {
    ...
    const choose = WhyChoose({
      right_image: "images/why_choose/" + right_image,
