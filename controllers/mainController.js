...
exports.flavours = async (req, resp) => {
  ...
  } else if (filter === "BOPP Bags") {
    const flr = { category: "BOPP Bags" };
    var pro = await Product.find(flr);
  } else if (filter === "Insecticides") {
    const flr = { category: "Insecticides" };
    var pro = await Product.find(flr);
  } else if (filter === "PP Laminated Bags") {
    const flr = { category: "PP Laminated Bags" };
    var pro = await Product.find(flr);
  } else {
    const flr = { category: "PP Wowen Fabric Roll" };
    var pro = await Product.find(flr);
  }
...
exports.add_cart = async (req, resp) => {
  ...
  for (let i = 0; i < req.session.cart.length; i++) {
    ...
    sub_total =
      sub_total +
      req.session.cart[i].product_price * 1 * req.session.cart[i].quantity;
...
exports.send_email = async (req, resp) => {
  ...
  let emailTemplate = fs.readFileSync(
    `${__dirname}/../emailTemplate/emailTemp.html`,
    "utf-8"
  );
